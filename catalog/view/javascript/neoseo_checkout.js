		...
		success: function (json) {
			if (json['error']) {
				...
				if (json['error']['warning']) {
					...
					$('#warning-messages').prepend('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + json['error'] + '<button type="button" class="close" data-dismiss="alert">×</button></div>');
					$('.warning').fadeIn('slow');
				}
...
function read_file(file)
{
	...
	for (var i = 0; i < file.length; i++)
	{
		...
		} else if (file[i].type.indexOf('openxmlformats-officedocument') !== -1)
		{
			...
			$('.button-uploading').after('<span class="error_files" style="color:#f00">{{error_upload_file}}<span>');
	}
}
