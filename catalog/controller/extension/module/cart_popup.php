<?php
class ControllerExtensionModuleCartPopup extends Controller {

  static $_ext_code    = 'cart_popup';

  public function index() {
    $data = array();

    // connect models array
    $models = array('catalog/product', 'tool/image', 'tool/upload', 'setting/extension', 'extension/module/'.self::$_ext_code);
    foreach ($models as $model) {
      $this->load->model($model);
    }

    $data = array_merge($data, $this->load->language('extension/module/'.self::$_ext_code), $this->config->get(self::$_ext_code.'_text'), $this->config->get(self::$_ext_code.'_setting'));

    $text  = (array)$this->config->get(self::$_ext_code.'_text');
    $setting  = (array)$this->config->get(self::$_ext_code.'_setting');

    $language_code = $this->session->data['language'];

    if (isset($text[$language_code])) {
      $data['heading_title'] = $text[$language_code]['heading'];
      $data['text_empty']   = html_entity_decode($text[$language_code]['empty_text'], ENT_QUOTES, 'UTF-8');
    }
    ...
    $data['text_title_bl'] = $this->language->get('text_title_bl');
    $data['text_text_bl'] = $this->language->get('text_text_bl');
    $data['by_one_click'] = $this->language->get('by_one_click');

    $data['current_url'] = (isset($this->request->server['HTTPS']) ? HTTPS_SERVER : HTTP_SERVER) . substr($this->request->server['REQUEST_URI'], 1, (strlen($this->request->server['REQUEST_URI'])-1));
    ...
    foreach ($this->cart->getProducts() as $product) {
      ...
      $image = ($product['image']) ? $this->model_tool_image->resize($product['image'], $setting['main_image_width'], $setting['main_image_height']) : $this->model_tool_image->resize("no_image.jpg", $setting['main_image_width'], $setting['main_image_height']);

      $crm_image = HTTPS_SERVER . 'image/' . $product['image'];
      ...
      if($product['old_price']){
        $old_price = (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) ? $this->currency->format($this->tax->calculate($product['old_price'], $product['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']) : false;
      }else{
        $old_price = false;
      }

      $price_int = (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) ? $this->tax->calculate($product['price'], $product['tax_class_id'], $this->config->get('config_tax')) : false;
      ...
      $data['products'][] = array(
        ...
        'price'       => $price,
        'price_int'   => $price_int,
        'old_price'   => $old_price ? $old_price : false,
        'tax'         => $tax,
        'tax_total'   => $tax_total,
        'total'       => $total,
    ...
    if ($setting['marketing_type'] == 2) {
      ...
      $upsell_data = $this->config->get(self::$_ext_code.'_upsell_data');
        ...
        if ($result['cart_products'] && $result['type']) {
          ...
          foreach ($result['cart_products'] as $pr_id) {
            ...
            if (in_array($pr_id, $products_in_cart)) {
              ...
              $upsell_products = array_slice($upsell_products, 0, $setting['limit_up_sell_products']);
            }
          }
        }
      }
      ...
      $ajax_products = $this->{'model_extension_module_'.self::$_ext_code}->getCrossSellProducts($data_x);

      foreach($ajax_products as $product) {
        if (in_array($product['product_id'], $upsell_products)) {

          $image = ($product['image']) ? $this->model_tool_image->resize($product['image'], $setting['sub_up_sell_images_width'], $setting['sub_up_sell_images_height']) : $this->model_tool_image->resize("no_image.jpg", $setting['sub_up_sell_images_width'], $setting['sub_up_sell_images_height']);
          $price = (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) ? $this->currency->format($this->tax->calculate($product['price'], $product['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']) : false;
      ...
      if (!empty($setting['check']) && $setting['check'] == 3) {

        $products = array();

        if (isset($setting['cross_sell_products'])) {
          foreach ($setting['cross_sell_products'] as $product_id) {
            $products[] = $product_id;
          }
        }
        ...
        $data['ajax_all_products'] = $setting['cross_sell_products'] ? count($setting['cross_sell_products']) : 0;
        ...
        foreach($ajax_products as $product) {
          if (in_array($product['product_id'], $products)) {
            ...
            $image = ($product['image']) ? $this->model_tool_image->resize($product['image'], $setting['sub_images_width'], $setting['sub_images_height']) : $this->model_tool_image->resize("no_image.jpg", $setting['sub_images_width'], $setting['sub_images_height']);
            $price = (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) ? $this->currency->format($this->tax->calculate($product['price'], $product['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']) : false;
            $special = ((float)$product['special']) ? $this->currency->format($this->tax->calculate($product['special'], $product['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']) : false;
        ...
        if ($setting['check']) {
          ...
          if ($setting['check'] == 1) {
            $data_x = array(
              'start'               => isset($this->request->request['start']) ? (int)$this->request->request['start'] : 0,
              'end'                 => isset($this->request->request['end']) ? (int)$this->request->request['end'] : 3,
              'filter_category_id'  => (!empty($setting['cross_sell_categories']) && $setting['check'] == 1) ? $setting['cross_sell_categories'] : false,
              'filter_sub_category' => true
            );

            $data['ajax_all_products'] = $this->{'model_extension_module_'.self::$_ext_code}->getCrossSellTotalProducts($data_x);
          }
          ...
          if ($setting['check'] == 2) {
            $data_x = array(
              'start'                  => isset($this->request->request['start']) ? (int)$this->request->request['start'] : 0,
              'end'                    => isset($this->request->request['end']) ? (int)$this->request->request['end'] : 3,
              'filter_manufacturer_id' => (!empty($setting['cross_sell_manufacturers']) && $setting['check'] == 2) ? $setting['cross_sell_manufacturers'] : false
            );
    ...
    $data['coupon'] = $this->load->controller('extension/module/'.self::$_ext_code.'/coupon_index');
    $data['voucher'] = $this->load->controller('extension/module/'.self::$_ext_code.'/voucher_index');
    $data['reward'] = $this->load->controller('extension/module/'.self::$_ext_code.'/reward_index');
    $data['shipping'] = $this->load->controller('extension/module/'.self::$_ext_code.'/shipping_index');

    $view = $this->load->view('extension/module/'.self::$_ext_code.'/'.self::$_ext_code.'_index', $data);
  ...
  public function coupon_index() {
    ...
    $data = array_merge($data, $this->load->language('extension/module/'.self::$_ext_code));

    $data['coupon'] = (isset($this->session->data['coupon'])) ? $this->session->data['coupon'] : '';

    return $this->load->view('extension/module/'.self::$_ext_code.'/'.self::$_ext_code.'_coupon', $data);
  ...
  public function coupon() {
    ...
    $this->load->language('extension/module/'.self::$_ext_code);
    $this->load->model('extension/total/coupon');

    $coupon = (isset($this->request->post['cp_coupon'])) ? $this->request->post['cp_coupon'] : '';

    $coupon_info = $this->{'model_extension_total_coupon'}->getCoupon($coupon);

    if (empty($this->request->post['cp_coupon'])) {
      $json['error'] = $this->language->get('error_cp_coupon_empty');

      unset($this->session->data['coupon']);
    } elseif ($coupon_info) {
      $this->session->data['coupon'] = $this->request->post['cp_coupon'];
      $json['success'] = $this->language->get('text_success_coupon');
    } else {
      $json['error'] = $this->language->get('error_cp_coupon');
    }
  ...
  public function voucher_index() {
    ...
    return $this->load->view('extension/module/'.self::$_ext_code.'/'.self::$_ext_code.'_voucher', $data);
  ...
  public function voucher() {
    ...
    $voucher = (isset ($this->request->post['cp_voucher'])) ? $this->request->post['cp_voucher'] : '';

    $voucher_info = $this->{'model_extension_total_voucher'}->getVoucher($voucher);

    if (empty($this->request->post['cp_voucher'])) {
      $json['error'] = $this->language->get('error_cp_voucher_empty');
    } elseif ($voucher_info) {
      $this->session->data['voucher'] = $this->request->post['cp_voucher'];
      $json['success'] = $this->language->get('text_success_voucher');
    } else {
      $json['error'] = $this->language->get('error_cp_voucher');
    }
  ...
  public function reward_index() {
    ...
    if ($points && $points_total) {
      ...
      $data['reward'] = isset($this->session->data['reward']) ? $this->session->data['reward'] : '';
       ...
       return $this->load->view('extension/module/'.self::$_ext_code.'/'.self::$_ext_code.'_reward', $data);
  ...
  public function reward() {
    ...
    if (empty($this->request->post['cp_reward'])) {
      $json['error'] = $this->language->get('error_cp_reward');
    }

    if ($this->request->post['cp_reward'] > $points) {
      $json['error'] = sprintf($this->language->get('error_cp_points'), $this->request->post['cp_reward']);
    }

    if ($this->request->post['cp_reward'] > $points_total) {
      $json['error'] = sprintf($this->language->get('error_cp_maximum'), $points_total);
    }

    if (!$json) {
      $this->session->data['reward'] = abs($this->request->post['cp_reward']);
  ...
  public function shipping_index() {
    ...
    if ($this->config->get('shipping_status') && $this->config->get('shipping_estimator') && $this->cart->hasShipping()) {
      ...
      return $this->load->view('extension/module/'.self::$_ext_code.'/'.self::$_ext_code.'_shipping', $data);
  ...
  public function shipping_quote() {
    ...
    if (!$this->cart->hasShipping()) {
      $json['error']['warning'] = sprintf($this->language->get('error_cp_no_shipping'), $this->url->link('information/contact', '', true));
    }

    if ($this->request->post['cp_country_id'] == '') {
      $json['error']['country'] = $this->language->get('error_cp_country');
    }

    if (!isset($this->request->post['cp_zone_id']) || $this->request->post['cp_zone_id'] == '') {
      $json['error']['zone'] = $this->language->get('error_cp_zone');
    }

    $this->load->model('localisation/country');

    $country_info = $this->model_localisation_country->getCountry($this->request->post['cp_country_id']);

    if ($country_info && $country_info['postcode_required'] && (utf8_strlen(trim($this->request->post['cp_postcode'])) < 2 || utf8_strlen(trim($this->request->post['cp_postcode'])) > 10)) {
      $json['error']['postcode'] = $this->language->get('error_cp_postcode');
    }

    if (!$json) {
      $this->tax->setShippingAddress($this->request->post['cp_country_id'], $this->request->post['cp_zone_id']);
      ...
      $zone_info = $this->model_localisation_zone->getZone($this->request->post['cp_zone_id']);
      ...
      $this->session->data['shipping_address'] = array(
        ...
        'address_1'      => '',
        'address_2'      => '',
        'postcode'       => $this->request->post['cp_postcode'],
        'city'           => '',
        'zone_id'        => $this->request->post['cp_zone_id'],
        'zone'           => $zone,
        'zone_code'      => $zone_code,
        'country_id'     => $this->request->post['cp_country_id'],
        'country'        => $country,
  ...
  public function shipping() {
    ...
    if (!empty($this->request->post['cp_shipping_method'])) {
      $shipping = explode('.', $this->request->post['cp_shipping_method']);

      if (!isset($shipping[0]) || !isset($shipping[1]) || !isset($this->session->data['shipping_methods'][$shipping[0]]['quote'][$shipping[1]])) {
        $json['error'] = $this->language->get('error_cp_shipping');
      }
    } else {
      $json['error'] = $this->language->get('error_cp_shipping');
    }
  ...
  public function initcart() {
    ...
    if (isset($text[$language_code])) {
      $json['text_in_cart'] = $text[$language_code]['call_button'];
      $json['text_in_cart_vs_options'] = $text[$language_code]['call_button_vs_options'];
    }

    if (empty($products)) {
      $json['error'] = html_entity_decode($text[$language_code]['empty_text'], ENT_QUOTES, 'UTF-8');
    }
    ...
    $json['add_function_selector'] = $setting['add_function_selector'];
    $json['add_id_selector'] = $setting['add_id_selector'];
  ...
  public function saveCart() {
    ...
    // connect models array
    $models = array('extension/module/'.self::$_ext_code, 'setting/extension', 'tool/image', 'tool/upload');
    foreach ($models as $model) {
      $this->load->model($model);
    }
    ...
    if (isset($this->request->request['type'])) {
      ...
      if ($this->request->request['type'] == 'wishlist') {
        ...
        if ($this->customer->isLogged()) {
          ...
          foreach ($this->cart->getProducts() as $product) {
            $this->model_account_wishlist->addWishlist($product['product_id']);
          }
        ...
        $visitor_data = $this->{'model_extension_module_'.self::$_ext_code}->getSavedCart($visitor_id);
        ...
        if (!$visitor_data) {
          ...
          $this->{'model_extension_module_'.self::$_ext_code}->addSavedCart($data_x);
        } else {
          $data_x = array(
            'cart'    => serialize($this->cart->getProducts()),
            'totals'  => $this->cart->getTotal(),
          ...
          $this->{'model_extension_module_'.self::$_ext_code}->updateSavedCart($data_x);
        }
      ...
      if ($this->request->request['type'] == 'email') {
        ...
        if (!isset($json['error'])) {
          ...
          $html = $this->load->view('mail/'.self::$_ext_code.'_mail', $data);
          ...
          if ($setting['new_saved_cart_admin_alert']) {
            $emails = explode(',', $setting['email_for_notification']);
