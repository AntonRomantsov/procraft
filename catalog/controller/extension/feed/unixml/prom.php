<?php
class ControllerExtensionFeedUnixmlProm extends Controller {
        ...
        for($startup['iteration'] = 0; 1; $startup['iteration']++){
          ...
          if($controller_data['products']){
            ...
            foreach($controller_data['products'] as $product_id => $product){
              ...
              $xml .= '<offer id="' . $product_id . '" available="' . ($product['stock']?'true':'') .'" in_stock="' . ($product['stock']?'true':'false') .'" selling_type="' . $seltype .'" ' . $group_id . '>';
