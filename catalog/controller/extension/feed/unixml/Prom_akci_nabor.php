<?php
class ControllerExtensionFeedUnixmlPromakcinabor extends Controller {
  public function index() {

    $feed = 'Prom_akci_nabor';
    ...
    if($controller_data['products']){
        ...
        foreach($controller_data['products'] as $product_id => $product){
            ...
            if($product['special']){
                ...
                $xml .= '<offer id="' . $product_id . '" available="' . ($product['stock']?'true':'false') .'" in_stock="' . ($product['stock']?'true':'false') .'" selling_type="' . $seltype .'" ' . $group_id . '>';
                ...
                // Вставляем специальную цену и старую цену
