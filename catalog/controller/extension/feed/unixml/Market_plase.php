<?php
class ControllerExtensionFeedUnixmlMarketplase extends Controller {
  public function index() {
    ...
    //XML_body
      ...
      $xml .= '<currencies>';
      $xml .= '<currency id="' . $startup['currency'] . '" rate="1"></currency>';
      $xml .= '</currencies>';
      ...
      if($startup['delivery_cost'] !== false) {
        $xml .= '<delivery-options><option cost="' . $startup['delivery_cost'] . '" days="' . $startup['delivery_time'] . '" order-before="' . $startup['delivery_jump'] . '"></option></delivery-options>';
      }
...
for($startup['iteration'] = 0; 1; $startup['iteration']++){
  ...
  if($controller_data['products']){
    foreach($controller_data['products'] as $product_id => $product){
      ...
      $xml .= '<vendorCode>' . $product['sku'] .  '</vendorCode>';
      $xml .= '<ean>' . $product['ean'] .  '</ean>';
      if($product['garanty_period'] == 0){
        $xml .= '<guarantee unit="days" type="' . $product['garanty_type'] . '">14</guarantee>';
      } else {
        $xml .= '<guarantee type="' . $product['garanty_type'] . '">' . $product['garanty_period'] . '</guarantee>';
      }
