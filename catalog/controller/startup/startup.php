...
class ControllerStartupStartup extends Controller {
	public function index() {
		...
		foreach($languages as $language){
			if(substr($language['code'], 0, 2) == $lang_code){
		...
		// Language Detection
		if (!empty($this->request->server['HTTP_ACCEPT_LANGUAGE']) && !array_key_exists($code, $languages) && $this->config->get('config_detect_browser_lang')) {
			$detect = '';
