    ...
    public function index() {
        ...
        foreach ($products as $product) {
            ...
            if ($product_info['special']) {
                ...
                $old_price = $this->simplecheckout->formatCurrency($this->tax->calculate($product['old_price'], $product['tax_class_id'], $this->config->get('config_tax')));
            }
        ...
        if ($currentTheme == 'shoppica' || $currentTheme == 'shoppica2') {
          $this->_templateData['cart_total'] = $this->simplecheckout->formatCurrency($total);
				...
				if ($this->customer->isLogged() && $this->config->get('total_discount_status')) {
					...
					$discounts_remainder = $this->config->get('config_discount');
					$discounts_remainder = explode('|', $discounts_remainder);
					$new_discounts_remainder = array();
					$new_percents_remainder = array();
					$new_values_remainder = array();
					foreach ($discounts_remainder as $discount_remainder) {
						$tmp = explode(':', $discount_remainder);
						$new_discounts_remainder[$tmp[0]] = $tmp[1];
						$new_percents_remainder[] = $tmp[0];
						$new_values_remainder[] = $tmp[1];
					}

					$new_discounts_remainder = array_reverse($new_discounts_remainder, true);
					$new_percents_remainder = array_reverse($new_percents_remainder);
					$new_values_remainder = array_reverse($new_values_remainder);

					$c = 0;

					foreach ($new_discounts_remainder as $discount_percent => $new_discount_remainder) {
						if ((float)$customer_order_total >= $new_discount_remainder) {
							$this->_templateData['old_customer_percent'] = $discount_percent;
							if (!$c) {
								$this->_templateData['new_customer_percent'] = false;
								$this->_templateData['change'] = false;
							} else {
								$this->_templateData['new_customer_percent'] = $new_percents_remainder[$c - 1];
								$this->_templateData['change'] = $this->currency->format($new_values_remainder[$c - 1] - $customer_order_total, $this->session->data['currency']);
								//$this->_templateData['customer_order_total'] =  $this->currency->format($this->model_account_customer->getOrderTotals($this->customer->getId()), $this->session->data['currency']);
							}
							$new_customer_percent = $this->_templateData['new_customer_percent'];
							$change = $this->_templateData['change'];
							break;
						}
						$c++;
					}
					...
					$this->_templateData['old_customer_percent'] = false;
					$this->_templateData['new_customer_percent'] = false;
					$this->_templateData['change'] = false;
				}

				$this->_templateData['text_next_discount'] = sprintf($this->language->get('text_next_discount'), $this->_templateData['new_customer_percent'] . '%', $this->_templateData['change']);

				$this->load->model('account/customer');

				if ($this->customer->isLogged() && $this->config->get('total_discount_status')) {
					$this->_templateData['customer_order_total'] = $this->model_account_customer->getOrderTotals($this->customer->getId());
					...
					$this->_templateData['steps'] = array();

					foreach ($new_discounts as $discount_percent => $new_discount) {
						$this->_templateData['steps'][$discount_percent] = $this->currency->format($new_discount, $this->session->data['currency']);
					}
					...
					$this->_templateData['background'] = array();

					foreach ($new_discounts as $discount_percent => $new_discount) {
						if ((float) $this->_templateData['customer_order_total'] >= $new_discount) {
							$this->_templateData['customer_percent'] = $discount_percent;
							$c = 0;
							foreach ($old_new_discounts as $old_discount_percent => $old_new_discount) {
								if ((float) $this->_templateData['customer_order_total'] >= $old_new_discount && (float) $this->_templateData['customer_order_total'] > $new_values[$c + 1]) {
									$this->_templateData['background'][$old_discount_percent] = 100;
								} elseif ((float) $this->_templateData['customer_order_total'] >= $old_new_discount && (float) $this->_templateData['customer_order_total'] < $new_values[$c + 1]) {
									$this->_templateData['background'][$old_discount_percent] = ((( $this->_templateData['customer_order_total'] - $old_new_discount) * 100) / ($new_values[$c + 1] - $old_new_discount));
								} else {
									$this->_templateData['background'][$old_discount_percent] = 0;
								}
								$c++;
							}
							break;
