...
class ControllerAccountOrder extends Controller {
	public function index() {
		...
		foreach ($results as $result) {
			...
			$data['cancelled_status'] = array(7, 8, 9, 10, 11, 14, 16);
			$data['success_status'] = array(3, 5);

			$this->load->model('catalog/product');
			$this->load->model('tool/upload');

			// Products
			$data['orders'][$i]['products'] = array();
			...
			foreach ($products as $product) {
				...
				if ($product_info['image']) {
					$data['prod_img'] = $this->model_tool_image->resize($product_info['image'], 60, 60);
				} else {
					$data['prod_img'] = '';
				}
				...
				$data['orders'][$i]['count_products'] += $product['quantity'];
			}
			$i++;    
		}
