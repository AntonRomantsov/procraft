...
class ControllerPaymentUkrcreditsPp extends Controller {
    ...
    public function senddatadeal(){
        ...
        if ($order_info) {
			...
			$data_deal['amount'] = round($sumtotal, 2);
        ...
        for($i = 0; $i < count($data_deal['products']); $i++){
            if($data_deal['products'][$i]['price'] == 0){
            	$data_deal['products'][$i]['price'] += 0.01;
            	$data_deal['amount'] += 0.01;
            }
        } 
        $data_deal['signature'] = $this->generateSignature($data_deal);
        $data_deal['amount'] = (string)$data_deal['amount'];
