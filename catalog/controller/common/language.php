	...
	private function seo_language() {
		...
		if (isset($this->request->post['redirect'])) {
			...
			if(isset($route)&& isset($url) && isset($protocol)) {
				$redirect_url = str_replace(array('/ru', '/ua'), '/' . substr($this->request->post['code'], 3, 2), $this->url->link($route, $url, $protocol));
				//$redirect_url = HTTPS_SERVER . substr($this->request->post['code'], 3, 2) . '/index.php?route=' . $route . $url;
			} else {
				$redirect_url = str_replace(array('/ru', '/ua'), '/' . substr($this->request->post['code'], 3, 2), $this->url->link('common/home'));
				//$redirect_url = HTTPS_SERVER . substr($this->request->post['code'], 3, 2) . '/index.php?route=common/home';
			}
