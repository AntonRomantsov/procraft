...
class ControllerPageSuccess extends Controller {
	public function index() {
		...
		if ($page_form_info) {
			...
			if(VERSION < '2.2.0.0') {
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/page/page_oc2/success.tpl')) {
					$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/page/page_oc2/success.tpl', $data));
				} else {
					$this->response->setOutput($this->load->view('default/template/page/page_oc2/success.tpl', $data));
				}
			} else if(VERSION <= '2.3.0.2') {
				$this->response->setOutput($this->load->view('page/page_oc2/success', $data));
			} else{
				$this->response->setOutput($this->load->view('page/page_oc3/success', $data));
			}
		} else {
