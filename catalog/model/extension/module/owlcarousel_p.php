<?php class ModelExtensionModuleOwlCarouselP extends Model {
    ...
    public function getProducts($data = array()) {
        ...
        if($this->use_cache) {
            ...
            $cachename = 'owlcarousel_p.getproducts.' . md5(http_build_query($data));
        ...
        if($data['white_list']){
           $sql .= " AND p.product_id IN (" . implode(',', $data['white_list']) . ")";
        } 

        if($data['black_list']){
           $sql .= " AND p.product_id NOT IN (" . implode(',', $data['black_list']) . ")";
        }        
    ...
    public function getCategoriesByParentId($category_id) {
        if($this->use_cache) {
            $cachename = 'owlcarousel_p.getcategoriesbyparentid.' . $category_id;
            $cache = $this->cache->get($cachename);
    ...
    public function getCategoriesByProductId($product_id) {
        if($this->use_cache) {
            $cachename = 'owlcarousel_p.getcategoriesbyproductid.' . $product_id;
            $cache = $this->cache->get($cachename);
            if (!$cache || !is_array($cache)) {
                $cache = array();
            }
    ...
    public function getProduct($product_id, $data = array()) {        
        ...
        if($this->use_cache) {
            $cachename = 'owlcarousel_p.getproduct.' . md5(http_build_query($data));
            $cache = $this->cache->get($cachename);
            if (!$cache || !is_array($cache)) {
                $cache = array();
            }
    ...
    function getSubCategories($category_ids){
        $query = $this->db->query("SELECT c.category_id, cd.name FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id) WHERE c.parent_id IN (" . implode(',', $category_ids) .  ") AND c.parent_id != 0 AND cd.language_id = " . (int)$this->config->get('config_language_id') . "");
        return $query->rows;
    }
