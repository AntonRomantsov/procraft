...
class ModelExtensionModuleCartPopup extends Model {
	...
	public function addSavedCart($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "cp_saved_carts SET cart_products = '" . $this->db->escape($data['cart']) . "', totals = '" . $data['totals'] . "', visitor = '" . $data['visitor'] . "', email = '" . $data['email'] . "', ip = '" . $data['ip'] . "', date_added = NOW()");
	}

	public function updateSavedCart($data) {
		$this->db->query("UPDATE " . DB_PREFIX . "cp_saved_carts SET cart_products = '" . $this->db->escape($data['cart']) . "', totals = '" . $data['totals'] . "', email = '" . $data['email'] . "' WHERE visitor = '" . $data['visitor'] . "'");
	}

	public function getSavedCart($visitor_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "cp_saved_carts WHERE visitor = '" . $visitor_id . "'");
