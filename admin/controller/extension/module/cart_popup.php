...
class ControllerExtensionModuleCartPopup extends Controller {
  ...
  public function index() {
    ...
    $models = array(
      ...
      'catalog/product', 
      'tool/image', 
      'extension/module/'.self::$_ext_code
    );
    ...
    foreach ($styles as $style) {
      $this->document->addStyle('view/stylesheet/'.self::$_ext_code.'/'.$style);
    }
    ...
    if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validate()) {
      ...
      if (isset($content)) {
        ...
        $this->model_setting_setting->editSetting(self::$_ext_code, unserialize($content));
        $this->response->redirect($this->url->link('marketplace/extension', 'user_token='.$this->session->data['user_token'].'&type=module', true));
      } else {
        $this->session->data['success'] = $this->language->get('text_success');
        $this->model_setting_setting->editSetting(self::$_ext_code, $this->request->post);
        $this->model_setting_setting->editSetting('module_' . self::$_ext_code, array('module_' . self::$_ext_code .'_status' => $this->request->post[self::$_ext_code.'_setting']['activate']));
        $this->response->redirect($this->url->link('marketplace/extension', 'user_token='.$this->session->data['user_token'].'&type=module', true));
      }
    }
    ...
    $data['breadcrumbs'] = array(
      ...
      2 => array(
        ...
        'href'      => $this->url->link('extension/module/'.self::$_ext_code, 'user_token='.$this->session->data['user_token'], true),
        'separator' => ' :: '
      )
    );

    $data['action']           = $this->url->link('extension/module/'.self::$_ext_code, 'user_token='.$this->session->data['user_token'], true);
    $data['action_plus']      = $this->url->link('extension/module/'.self::$_ext_code.'/edit_and_stay', 'user_token='.$this->session->data['user_token'], true);
    $data['export_settings_button'] = $this->url->link('extension/module/'.self::$_ext_code.'/export_settings', 'user_token='.$this->session->data['user_token'], true);
    $data['import_settings_button'] = $this->url->link('extension/module/'.self::$_ext_code.'/import_settings', 'user_token='.$this->session->data['user_token'], true);
    $data['cancel']           = $this->url->link('marketplace/extension', 'user_token='.$this->session->data['user_token'].'&type=module', true);
    $data['admin_language']   = $this->config->get('config_admin_language');
    $data['_ext_code']     = (string)self::$_ext_code;
    $data['user_token']            = $this->session->data['user_token'];
    $data['text']        = isset($this->request->post[self::$_ext_code.'_text']) ? $this->request->post[self::$_ext_code.'_text'] : $this->config->get(self::$_ext_code.'_text');
    $data['setting']        = isset($this->request->post[self::$_ext_code.'_setting']) ? $this->request->post[self::$_ext_code.'_setting'] : $this->config->get(self::$_ext_code.'_setting');
    ...
    } elseif (!empty($setting['cross_sell_products'])) {
      $products = $setting['cross_sell_products'];
    } else {
      $products = array();
    }
    ...
    } elseif (!empty($setting['cross_sell_categories'])) {
      $categories = $setting['cross_sell_categories'];
    } else {
      $categories = array();
    }
    ...
    } elseif (!empty($setting['cross_sell_manufacturers'])) {
      $manufacturers = $setting['cross_sell_manufacturers'];
    } else {
      $manufacturers = array();
    }
    ...
    if (isset($this->request->post[self::$_ext_code . '_upsell_data'])) {
      $upsell_array = $this->request->post[self::$_ext_code . '_upsell_data'];
    } elseif ($this->config->get(self::$_ext_code . '_upsell_data')) {
      $upsell_array = $this->config->get(self::$_ext_code . '_upsell_data');
    } else {
      $upsell_array = array();
    }
    ...
    $data['cp_products'] = $this->{'model_extension_module_'.self::$_ext_code}->getcpCatalog();
    ...
    $this->response->setOutput($this->load->view('extension/module/'.self::$_ext_code, $data));
  }
  ...
  public function history() {
    ...
    $models = array('extension/module/'.self::$_ext_code, 'tool/image');
    foreach ($models as $model) {
      $this->load->model($model);
    }
    ...
    $results = $this->{'model_extension_module_'.self::$_ext_code}->getSavedCartArray(($page - 1) * 10, 10);
    ...
    $history_total = $this->{'model_extension_module_'.self::$_ext_code}->getTotalSavedCartArray();

    $pagination = new Pagination();
    $pagination->total = $history_total;
    $pagination->page = $page;
    $pagination->limit = 10;
    $pagination->url = $this->url->link('extension/module/'.self::$_ext_code.'/history', 'user_token='.$this->session->data['user_token'].'&page={page}', true);

    $data['pagination'] = $pagination->render();

    $data['results'] = sprintf($this->language->get('text_pagination'), ($history_total) ? (($page - 1) * 10) + 1 : 0, ((($page - 1) * 10) > ($history_total - 10)) ? $history_total : ((($page - 1) * 10) + 10), $history_total, ceil($history_total / 10));

    $this->response->setOutput($this->load->view('extension/module/'.self::$_ext_code.'_history', $data));
  }
  ...
  public function delete_selected() {
    ...
    $models = array('extension/module/'.self::$_ext_code);
    foreach ($models as $model) {
      $this->load->model($model);
    }

    $info = $this->{'model_extension_module_'.self::$_ext_code}->getSavedCart((int)$this->request->get['delete']);

    if ($info) {
      $this->{'model_extension_module_'.self::$_ext_code}->deleteSavedCart((int)$this->request->get['delete']);
    }
  ...
  public function delete_all() {
    ...
    $this->{'model_extension_module_'.self::$_ext_code}->deleteAllSavedCarts();
  ...
  public function delete_all_selected() {
    ...
    if (isset($this->request->request['selected'])) {
      foreach ($this->request->request['selected'] as $link_id) {
        $info = $this->{'model_extension_module_'.self::$_ext_code}->getSavedCart((int)$link_id);

        if ($info) {
          $this->{'model_extension_module_'.self::$_ext_code}->deleteSavedCart((int)$link_id);
        }
      }
    }
  ...
  public function edit_and_stay() {
    ...
    if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validate()) {
      ...
      if (isset($content)) {
        $this->session->data['success'] = $this->language->get('text_success');
        $this->model_setting_setting->editSetting(self::$_ext_code, unserialize($content));
        $this->response->redirect($this->url->link('extension/module/'.self::$_ext_code, 'user_token='.$this->session->data['user_token'], true));
      } else {
        $this->session->data['success'] = $this->language->get('text_success');
        $this->model_setting_setting->editSetting(self::$_ext_code, $this->request->post);
        $this->response->redirect($this->url->link('extension/module/'.self::$_ext_code, 'user_token='.$this->session->data['user_token'], true));
  ...
  public function install() {
    ...
    $text_form = $this->load->language('extension/module/'.self::$_ext_code);

    $models = array('setting/setting', 'setting/extension', 'extension/module/'.self::$_ext_code);
    foreach ($models as $model) {
      $this->load->model($model);
    }

    $modules = array('extension/module/'.self::$_ext_code);
    ...
    $this->{'model_extension_module_'.self::$_ext_code}->createDBTables();

    $this->model_setting_setting->editSetting(self::$_ext_code, array(
      self::$_ext_code.'_text' => array(
      ...
      self::$_ext_code.'_setting' => array(
        ...
        'front_ext_code'                      => str_replace(array('<b>','</b>'), "", $text_form['heading_name']),
      )
    ));

    if (!in_array(self::$_ext_code, $this->model_setting_extension->getInstalled('module'))) {
      $this->model_setting_extension->install('module', self::$_ext_code);
    }

    $this->model_setting_setting->editSetting('module_' . self::$_ext_code, array('module_' . self::$_ext_code . '_status' => 1));
  ...
  public function uninstall() {
    ...
    $this->{'model_extension_module_'.self::$_ext_code}->deleteDBTables();
    $this->model_setting_extension->uninstall('module', self::$_ext_code);
    $this->model_setting_setting->deleteSetting(self::$_ext_code);
    $this->model_setting_setting->deleteSetting('module_' . self::$_ext_code);
  }
  ...
  private function validate() {
    ...
    if (!$this->user->hasPermission('modify', 'extension/module/'.self::$_ext_code)) {
      $this->error['warning'] = $text_form['error_permission'];
    }

    foreach ($this->request->post[self::$_ext_code.'_text'] as $language_code => $value) {
    ...
    foreach ($this->request->post[self::$_ext_code.'_setting'] as $main_key => $field) {
  ...
  public function get_background() {
    ...
    $check_dir = self::$_ext_code.'/background';
  ...
  public function export_settings() {
    ...
    $module_settings = $this->model_setting_setting->getSetting(self::$_ext_code);

    $this->response->addheader('Pragma: public');
    $this->response->addheader('Expires: 0');
    $this->response->addheader('Content-Description: File Transfer');
    $this->response->addheader('Content-Type: application/octet-stream');
    $this->response->addheader('Content-Disposition: attachment; filename='.self::$_ext_code.'_'.date("Y-m-d H:i:s", time()).'.txt');
