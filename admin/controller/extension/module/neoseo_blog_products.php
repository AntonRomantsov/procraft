	...
	public function index()
	{
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/' . $this->_route . '/' . $this->_moduleSysName(), 'user_token=' . $this->session->data['user_token'], 'SSL')
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title_raw'),
				'href' => $this->url->link('extension/' . $this->_route . '/' . $this->_moduleSysName(), 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}
		...
		$data['product_list_types'] = array(
			'column' => $this->language->get("text_column"),
		...
		if (file_exists(DIR_CATALOG . 'view/theme/' . $this->config->get('config_theme') . '/template/extension/' . $this->_route . '/' . $this->_moduleSysName() . '_grid.twig')) {
		...
		if (isset($this->request->post['selected_articles'])) {
			$data['selected_articles'] = $this->request->post['selected_articles'];
		} elseif (!empty($module_info) && isset($module_info['selected_articles'])) {
			$data['selected_articles'] = $module_info['selected_articles'];
		} else {
			$data['selected_articles'] = array();
		}
		...
		$data['categories'] = $this->{"model_blog_" . $this->_moduleSysName . '_category'}->getCategories(array('parent_id' => 0));
