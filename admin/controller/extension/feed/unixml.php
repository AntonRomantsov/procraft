...
class ControllerExtensionFeedUnixml extends Controller {
  ...
  //initUniXML - инициализация модуля
    ...
    private function initUniXML() {
      ...
      $text = "<div style=\"font-size:22px;text-align:center;margin-top:20px;\"><h3><strong>К сожалению UniXML не смог найти ключ лицензии на этот домен</strong></h3>
               <small style=\"display:block;margin:10px 0;font-size:16px;\">Такое бывает, ничего страшного</small>
               <div class=\"alert alert-info\" style=\"font-size:14px;line-height:16px;\">Чтобы вручную забрать ключ перейдите по ссылке: <a target=\"_blank\" href=\"https://unixml.pro/key?s=" . base64_encode($init_data) . "\">https://unixml.pro/key</a> и вставьте его в поле ниже</div>
  ...
  //getMdpKey
    private function getMdpKey($dmn_data = false){
      ...
      $curl = curl_init();
      curl_setopt($curl, CURLOPT_URL, "https://test.com");
  ...
  //index - старт админки
    public function index() {
      ...
      foreach(array($this->language->get('text_home') => 'common/dashboard',$this->language->get('text_extension') => ((substr(VERSION, 0, 1) == 3)?'marketplace/extension':'extension/extension'),$data['heading_title_module'] => $this->unixml->path) as $breadcrumb_text => $breadcrumb_link){
        $data['breadcrumbs'][] = array(
      ...
      public function importUploadFile() {
        ...
        if (!empty($this->request->files['file']['name']) && is_file($this->request->files['file']['tmp_name'])) {
          ...
          if (!in_array(strtolower(substr(strrchr($filename, '.'), 1)), array('xml','yml','txt', 'xls', '.xlsx'))) {
            $json['error'] = 'Вы можете загрузить только XML (YML), XLS формат файла';
          }
          ...
          if(in_array('replace_category', $sets)){
            ...
            if($data['replace_category']){
              ...
              foreach($query->rows as $row){
                ...
                $ocname = 'НЕ ЗАГРУЖАТЬ товары этой категории';
                if($row['oc']){
