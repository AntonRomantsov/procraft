...
class ControllerPagePageForm extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('page/page_form', $this->module_token .'=' . $this->ci_token . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('page/page_form', $this->module_token .'=' . $this->ci_token . $url, true)
		);

		$data['add'] = $this->url->link('page/page_form/add', $this->module_token .'=' . $this->ci_token . $url, true);
		$data['delete'] = $this->url->link('page/page_form/delete', $this->module_token .'=' . $this->ci_token . $url, true);
		$data['copy'] = $this->url->link('page/page_form/copy', $this->module_token .'=' . $this->ci_token . $url, true);
		...
		foreach ($results as $result) {
			$data['page_forms'][] = array(
				...
				'status_class' 		=> ($result['status']) ? 'label-success' : 'label-warning',
				'status' 		=> ($result['status']) ? $this->language->get('text_enabled') : $this->language->get('text_disabled'),
				'sort_order' 	=> $result['sort_order'],
				'edit'       	=> $this->url->link('page/page_form/edit', $this->module_token .'=' . $this->ci_token . '&page_form_id=' . $result['page_form_id'] . $url, true)
			);
		}
		...
		$data['sort_id'] = $this->url->link('page/page_form', $this->module_token .'=' . $this->ci_token . '&sort=p.page_form_id' . $url, true);
		$data['sort_title'] = $this->url->link('page/page_form', $this->module_token .'=' . $this->ci_token . '&sort=pd.title' . $url, true);
		$data['sort_sort_order'] = $this->url->link('page/page_form', $this->module_token .'=' . $this->ci_token . '&sort=p.sort_order' . $url, true);
		$data['sort_status'] = $this->url->link('page/page_form', $this->module_token .'=' . $this->ci_token . '&sort=p.status' . $url, true);
		...
		$pagination->url = $this->url->link('page/page_form', $this->module_token .'=' . $this->ci_token . $url . '&page={page}', true);
		...
		if(VERSION <= '2.3.0.2') {
			$this->response->setOutput($this->load->view('page/page_list.tpl', $data));
		} else {
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['page_form_id'])) {
			$data['action'] = $this->url->link('page/page_form/add', $this->module_token .'=' . $this->ci_token . $url, true);
		} else {
			$data['action'] = $this->url->link('page/page_form/edit', $this->module_token .'=' . $this->ci_token . '&page_form_id=' . $this->request->get['page_form_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('page/page_form', $this->module_token .'=' . $this->ci_token . $url, true);
		...
		if (isset($this->request->post['page_form_product'])) {
			...
			$page_form_products = $this->model_page_page_form->getPageFormProducts($this->request->get['page_form_id']);
		} else {
		...
		if(VERSION <= '2.3.0.2') {
			$this->response->setOutput($this->load->view('page/page_form.tpl', $data));
		} else {
			$file_variable = 'template_engine';
			$file_type = 'template';
			$this->config->set($file_variable, $file_type);
	...
	protected function validateForm() {
		...
		if (isset($this->request->post['page_form_field'])) {
			foreach ($this->request->post['page_form_field'] as $row => $description) {
				...
				if(isset($description['option_value']) && !in_array($description['type'], array('select', 'multi_select', 'radio', 'radio_toggle', 'checkbox', 'checkbox_toggle')) ) {
	...
	public function copy() {
		...
		if (isset($this->request->post['selected']) && $this->validateCopy()) {
			foreach ($this->request->post['selected'] as $page_form_id) {
				$this->model_page_page_form->copyPageForm($page_form_id);
			}
