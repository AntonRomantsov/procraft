...
class ControllerPagePageRequest extends Controller {
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			$this->response->redirect($this->url->link('page/page_request', $this->module_token .'=' . $this->ci_token . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('page/page_request', $this->module_token .'=' . $this->ci_token . $url, true)
		);

		$data['add'] = $this->url->link('page/page_request/add', $this->module_token .'=' . $this->ci_token . $url, true);
		$data['delete'] = $this->url->link('page/page_request/delete', $this->module_token .'=' . $this->ci_token . $url, true);
		...
		foreach ($results as $result) {
			$data['page_requests'][] = array(
				...
				'view'           	=> $this->url->link('page/page_request/info', $this->module_token .'=' . $this->ci_token . '&page_request_id=' . $result['page_request_id'] . $url, true)
			);
		}
		...
		$data['sort_title'] = $this->url->link('page/page_request', $this->module_token .'=' . $this->ci_token . '&sort=pg.page_form_title' . $url, true);
		$data['sort_customer'] = $this->url->link('page/page_request', $this->module_token .'=' . $this->ci_token . '&sort=customer' . $url, true);
		$data['sort_ip'] = $this->url->link('page/page_request', $this->module_token .'=' . $this->ci_token . '&sort=pg.ip' . $url, true);
		$data['sort_date_added'] = $this->url->link('page/page_request', $this->module_token .'=' . $this->ci_token . '&sort=pg.date_added' . $url, true);
		...
		$pagination->url = $this->url->link('page/page_request', $this->module_token .'=' . $this->ci_token . $url . '&page={page}', true);
		...
		if(VERSION <= '2.3.0.2') {
			$this->response->setOutput($this->load->view('page/page_request_list.tpl', $data));
		} else {
	...
	public function info() {
		...
		if ($page_request_info) {
			...
			$data['back'] = $this->url->link('page/page_request', $this->module_token .'=' . $this->ci_token . $url, true);
			...
			if ($page_request_info['page_form_id']) {
				$data['page_form_href'] = $this->url->link('page/page_form/edit', $this->module_token .'=' . $this->ci_token . '&page_form_id=' . $page_request_info['page_form_id'], true);
			} else {
				$data['page_form_href'] = '';
			}
			...
			if(VERSION <= '2.3.0.2') {
				$this->response->setOutput($this->load->view('page/page_request_info.tpl', $data));
			} else {
				$file_variable = 'template_engine';
				$file_type = 'template';
				$this->config->set($file_variable, $file_type);
