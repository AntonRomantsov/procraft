...
class ControllerCommonFileManager extends Controller {
	public function index() {
		...
		if (substr(str_replace('\\', '/', realpath($directory) . '/' . $filter_name), 0, strlen(DIR_IMAGE . 'catalog')) == str_replace('\\', '/', DIR_IMAGE . 'catalog')) {
			...
			// Get files
			$files = glob($directory . '/' . $filter_name . '*.{jpg,jpeg,png,gif,JPG,JPEG,PNG,GIF,svg,SVG,pdf,PDF}', GLOB_BRACE);
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				$data['images'][] = array(
					...
					'thumb' => (strrchr($image, '.') != ".pdf") ? $this->model_tool_image->resize(utf8_substr($image, utf8_strlen(DIR_IMAGE)), 100, 100) : $this->model_tool_image->resize('pdf_icon.png', 100, 100),
					'name'  => implode(' ', $name),
					'type'  => 'image',
					'path'  => utf8_substr($image, utf8_strlen(DIR_IMAGE)),
