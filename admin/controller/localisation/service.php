<?php
class ControllerLocalisationService extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('localisation/service');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('localisation/service', 'user_token=' . $this->session->data['user_token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] =   array(
			...
			'href' =>  $this->url->link('localisation/service', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);

		$data['add'] = $this->url->link('localisation/service/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('localisation/service/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['locations'][] =   array(
				...
				'edit'        => $this->url->link('localisation/service/edit', 'user_token=' . $this->session->data['user_token'] . '&location_id=' . $result['location_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('localisation/service', 'user_token=' . $this->session->data['user_token'] . '&sort=name' . $url, true);
		$data['sort_address'] = $this->url->link('localisation/service', 'user_token=' . $this->session->data['user_token'] . '&sort=address' . $url, true);
		...
		$pagination->url = $this->url->link('localisation/service', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['location_id'])) {
			$data['action'] = $this->url->link('localisation/service/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('localisation/service/edit', 'user_token=' . $this->session->data['user_token'] .  '&location_id=' . $this->request->get['location_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('localisation/service', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		if (isset($this->request->post['service'])) {
			...
			$data['service'] = $location_info['service'];
		} else {
			$data['service'] = 1;
		}
	...
	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'localisation/service')) {
