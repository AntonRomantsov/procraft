<?php 
class ModelExtensionModuleFacebooklogin extends Model {
	...
	public function duplicatedModuleName($name, $module_id=0) {
		if(!empty($module_id)) {
			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "module` WHERE `code` = 'facebooklogin' AND `name` = '" . $this->db->escape($name) . "' AND `module_id` != '" . $this->db->escape($module_id) . "'");
			return empty($query->rows);
		} else {
			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "module` WHERE `code` = 'facebooklogin' AND `name` = '" . $this->db->escape($name) . "'");
			return empty($query->rows);
		}		
	}
	...
	public function getModuleLayouts($store_id = 0) {
		$data = array();
		$query = $this->db->query("SELECT oc_layout.name, oc_layout_module.position FROM oc_layout_module 
			JOIN oc_layout
			ON oc_layout_module.layout_id = oc_layout.layout_id	z
			JOIN oc_layout_route
			ON oc_layout_module.layout_id = oc_layout_route.layout_id
			WHERE oc_layout_module.code like '%facebooklogin%'
			AND oc_layout_route.store_id = ".$store_id);
