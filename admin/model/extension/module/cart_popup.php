<?php
class ModelExtensionModuleCartPopup extends Model {

	static  $_ext_code       = 'cart_popup';

	public function getSavedCart($cart_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM `".DB_PREFIX."cp_saved_carts` WHERE cart_id = '".(int)$cart_id."'");

		return $query->row;
	}

	public function deleteSavedCart($cart_id) {
		$this->db->query("DELETE FROM `".DB_PREFIX."cp_saved_carts` WHERE cart_id = '".(int)$cart_id."'");
	}

	public function deleteAllSavedCarts() {
		$this->db->query("DELETE FROM `".DB_PREFIX."cp_saved_carts`");
	}
	...
	public function getSavedCartArray($start = 0, $limit = 10) {
		...
		$query = $this->db->query("SELECT * FROM `".DB_PREFIX."cp_saved_carts` ORDER BY date_added DESC LIMIT ".(int)$start.",".(int)$limit);

		return $query->rows;
	}

	public function getTotalSavedCartArray() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `".DB_PREFIX."cp_saved_carts`");

		return $query->row['total'];
	}

	public function createDBTables() {
		$sql  = "CREATE TABLE IF NOT EXISTS `".DB_PREFIX."cp_saved_carts` ( ";
	...
	public function deleteDBTables() {
		$sql  = "DROP TABLE IF EXISTS `".DB_PREFIX."cp_saved_carts`;";

		$this->db->query($sql);
	}

	public function getcpCatalog() {
		$catalog = array();
		return $catalog;
	}
}
