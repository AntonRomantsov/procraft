...
class ModelCatalogAttribute extends Model {
	...
	public function getAttribute($attribute_id) {
		$query = $this->db->query("SELECT a.*, ad.*, ag.parts as parts FROM " . DB_PREFIX . "attribute a LEFT JOIN " . DB_PREFIX . "attribute_description ad ON (a.attribute_id = ad.attribute_id) LEFT JOIN " . DB_PREFIX . "attribute_group ag ON (a.attribute_group_id = ag.attribute_group_id) WHERE a.attribute_id = '" . (int)$attribute_id . "' AND ad.language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function getAttributes($data = array()) {
		...
		//$sql = "SELECT *, (SELECT agd.name FROM " . DB_PREFIX . "attribute_group_description agd WHERE agd.attribute_group_id = a.attribute_group_id AND agd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS attribute_group FROM " . DB_PREFIX . "attribute a LEFT JOIN " . DB_PREFIX . "attribute_description ad ON (a.attribute_id = ad.attribute_id) LEFT JOIN " . DB_PREFIX . "attribute_group ag ON (a.attribute_group_id = ag.attribute_group_id) WHERE ag.parts = '" . (int)((isset($data['type']) && $data['type'] == 'parts') ? 1 : 0) . "' AND ad.language_id = '" . (int)$this->config->get('config_language_id') . "'";
