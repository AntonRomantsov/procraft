...
$('#button-filter').on('click', function() {
  url = 'index.php?route=page/page_request&<?php echo $module_token; ?>=<?php echo $ci_token; ?>';
...
$('input[name=\'filter_page_form_title\']').autocomplete({
  'source': function(request, response) {
    $.ajax({
      url: 'index.php?route=page/page_form/autocomplete&<?php echo $module_token; ?>=<?php echo $ci_token; ?>&filter_title=' +  encodeURIComponent(request),
      dataType: 'json',
      success: function(json) {
        response($.map(json, function(item) {
          return {
