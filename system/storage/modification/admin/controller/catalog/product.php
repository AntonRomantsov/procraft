...
class ControllerCatalogProduct extends Controller {
    ...
    if (isset($this->request->get['filter_import_batch'])) {
      $url .= '&filter_import_batch=' . urlencode(html_entity_decode($this->request->get['filter_import_batch'], ENT_QUOTES, 'UTF-8'));
		...
		if (isset($this->request->post['materials_instruction'])) {
			$data['materials_instruction'] = $this->request->post['materials_instruction'];
		} elseif (!empty($product_info)) {
			$data['materials_instruction'] = $product_info['materials_instruction'];
		} else {
			$data['materials_instruction'] = '';
		}

		if (isset($this->request->post['materials_draw'])) {
			$data['materials_draw'] = $this->request->post['materials_draw'];
		} elseif (!empty($product_info)) {
		...
		if (isset($this->request->post['garanty_id'])) {
			$data['garanty_id'] = $this->request->post['garanty_id'];
		} elseif (!empty($product_info)) {
			$data['garanty_id'] = $product_info['garanty_id'];
		} else {
			$data['garanty_id'] = 0;
		}

		$data['garanties'] = $this->model_catalog_product->getGaranties();
		...
		if ($this->config->get('module_giftor_status') && !empty($this->request->get['product_id'])) {
			$this->load->model('extension/module/giftor');
			$data['giftor_ids'] = $this->model_extension_module_giftor->getProductGiftsIds($this->request->get['product_id']);

			if (isset($this->request->post['product_giftor'])) {
				$products_giftor = $this->request->post['product_giftor'];
			} elseif (isset($this->request->get['product_id'])) {
				$products_giftor = $this->model_extension_module_giftor->getGiftsByProduct($this->request->get['product_id']);
			} else {
				$products_giftor = array();
			}

			$data['product_giftors'] = array();

			foreach ($products_giftor as $product_giftor) {
				$giftor_info = $this->model_catalog_product->getProduct($product_giftor['product_id']);

				if ($giftor_info) {
					$data['product_giftors'][] = array(
						'product_id' => $giftor_info['product_id'],
						'name'       => $giftor_info['name']
					);
				}
			}
		} else {
			$data['giftor_ids'] = array();
			$data['product_giftors'] = array();
		}
		...
		if (isset($this->request->post['product_kit_part'])) {
			$products = $this->request->post['product_kit_part'];
		} elseif (isset($this->request->get['product_id'])) {
			$products = $this->model_catalog_product->getKitParts($this->request->get['product_id']);
		} else {
			$products = array();
		}

		$data['kit_parts'] = array();

		foreach ($products as $product_id) {
			$kit_part_info = $this->model_catalog_product->getProduct($product_id);

			if ($kit_part_info) {
				$data['kit_parts'][] = array(
					'product_id' => $kit_part_info['product_id'],
					'name'       => $kit_part_info['name']
				);
			}
		}

		$data['entry_kit_part'] = $this->language->get('entry_kit_part');
		$data['help_kit_part'] = $this->language->get('help_kit_part');
    ...
    // OCFilter end
		...
		$data['entry_garanty'] = $this->language->get('entry_garanty');
