...
class ModelCustomerCustomer extends Model {
	public function addCustomer($data) {
      ...
      if (!empty($data['ip']) && defined('GKD_UNIV_IMPORT')) {
        $univimp_extra .= 'ip = "' . $this->db->escape($data['ip']) . '", ';
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET " . $univimp_extra . " customer_group_id = '" . (int)$data['customer_group_id'] . "', manager_id = '" . (int)$data['manager_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : json_encode(array())) . "', newsletter = '" . (int)$data['newsletter'] . "', salt = '" . ((defined('GKD_UNIV_IMPORT') && isset($data['salt'])) ? $this->db->escape($salt = $data['salt']) : $this->db->escape($salt = token(9))) . "', password = '" . ((defined('GKD_UNIV_IMPORT') && isset($data['salt'])) ? $this->db->escape($data['password']) : $this->db->escape(sha1($salt . sha1($salt . sha1($data['password']))))) . "', status = '" . (int)$data['status'] . "', safe = '" . (int)$data['safe'] . "', date_added = NOW()");
	...
	public function getManagers() {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "user WHERE is_manager = 1");
