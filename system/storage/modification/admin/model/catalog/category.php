      ...
      if (!empty($data['gkd_extra_fields']) && defined('GKD_UNIV_IMPORT')) {
        foreach ($data['gkd_extra_fields'] as $extra_field) {
          if (isset($data[$extra_field])) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "category SET " . $univimp_extra . " parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', garanty = '" . (int)$data['garanty'] . "', video = '" . $this->db->query($data['video']) . "', date_modified = NOW(), date_added = NOW()");
      ...
      if (!empty($data['gkd_extra_fields']) && defined('GKD_UNIV_IMPORT')) {
        foreach ($data['gkd_extra_fields'] as $extra_field) {
          if (isset($data[$extra_field])) {
			...
			$this->db->query("UPDATE " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', " . $univimp_extra . " `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', garanty = '" . (int)$data['garanty'] . "', video = '" . $this->db->escape($data['video']) . "', date_modified = NOW() WHERE category_id = '" . (int)$category_id . "'");
