      ...
      if (!empty($data['gkd_extra_fields']) && defined('GKD_UNIV_IMPORT')) {
        foreach ($data['gkd_extra_fields'] as $extra_field) {
          if (isset($data[$extra_field])) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "product SET " . $univimp_extra . " model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', garanty_id = '" . (int)$data['garanty_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW(), date_modified = NOW(), materials_instruction = '" . $this->db->escape($data['materials_instruction']) . "', materials_draw = '" . $this->db->escape($data['materials_draw']) . "'");
...
if (isset($data['partscount_pp'])) {
	if ($data['partscount_pp'] || $data['partscount_ii'] || $data['partscount_mb'] || $data['partscount_ab'] || $data['product_pp'] || $data['product_ii'] || $data['product_mb'] || $data['product_ab'] || $data['markup_pp'] || $data['markup_ii'] || $data['markup_mb'] || $data['markup_ab']) {
		...
		if (!empty($data['giftor_ids'])) {
			foreach ($data['giftor_ids'] as $gift_id) {
		...
		if (!empty($data['product_giftor'])) {
			foreach ($data['product_giftor'] as $gift_product_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "giftor SET gift_product_id = '" . (int)$gift_product_id . "', based = '1', `condition` = '0', min_total = '0', max_total = '0', date_start = '', date_end = '', customer_group = '" . json_encode(array("0")) . "', status = '1'");
      ...
      // Universal Import/Export - apply filters to cateogories
		...
		if (isset($data['product_kit_part'])) {
			foreach ($data['product_kit_part'] as $part_id) {
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_kit_part WHERE product_id = '" . (int)$product_id . "' AND part_id = '" . (int)$part_id . "'");
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_kit_part SET product_id = '" . (int)$product_id . "', part_id = '" . (int)$part_id . "'");
				// $this->db->query("DELETE FROM " . DB_PREFIX . "product_related WHERE product_id = '" . (int)$related_id . "' AND related_id = '" . (int)$product_id . "'");
				// $this->db->query("INSERT INTO " . DB_PREFIX . "product_related SET product_id = '" . (int)$related_id . "', related_id = '" . (int)$product_id . "'");
			}
		}
...
if (isset($data['partscount_pp'])) {
	if ($data['partscount_pp'] || $data['partscount_ii'] || $data['partscount_mb'] || $data['product_pp'] || $data['product_ii'] || $data['product_mb'] || $data['markup_pp'] || $data['markup_ii'] || $data['markup_mb']) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', garanty_id = '" . (int)$data['garanty_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_modified = NOW(), materials_instruction = '" . $this->db->escape($data['materials_instruction']) . "', materials_draw = '" . $this->db->escape($data['materials_draw']) . "' WHERE product_id = '" . (int)$product_id . "'");
      ...
      // Universal Import/Export - apply filters to cateogories
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_kit_part WHERE product_id = '" . (int)$product_id . "'");
		//$this->db->query("DELETE FROM " . DB_PREFIX . "product_related WHERE related_id = '" . (int)$product_id . "'");
    ...
    // OCFilter end
			...
			$data['product_kit_part'] = $this->getKitParts($product_id);
	...
	public function getKitParts($product_id) {
		$product_kit_part_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_kit_part WHERE product_id = '" . (int)$product_id . "'");

		foreach ($query->rows as $result) {
			$product_kit_part_data[] = $result['part_id'];
		}

		return $product_kit_part_data;
	}
	...
	public function getTotalProductsByGarantyId($garanty_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "product_ WHERE garanty_id = '" . (int)$garanty_id . "'");
	...
	public function getGaranties(){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "garanty g LEFT JOIN " . DB_PREFIX . "garanty_description gd ON (g.garanty_id = gd.garanty_id) WHERE gd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
		return $query->rows;
	}
}
