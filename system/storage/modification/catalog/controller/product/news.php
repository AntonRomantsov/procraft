...
class ControllerProductNews extends Controller {
	public function index() {
		$this->load->language('product/news');
		...
		$product_total = $this->model_catalog_product->getTotalProductNews();

		$results = $this->model_catalog_product->getProductNews($filter_data);
		...
		if(count($results) < 20){
		    ...
		    $results = $this->model_catalog_product->getProductLast($filter_data);
		}
		...
		$data['sorts'][] = array(
			...
			'href'  => $this->url->link('product/news', 'sort=p.sort_order&order=ASC' . $url)
		);

		$data['sorts'][] = array(
			'text'  => $this->language->get('text_name_asc'),
			'value' => 'pd.name-ASC',
			'href'  => $this->url->link('product/news', 'sort=pd.name&order=ASC' . $url)
		);

		$data['sorts'][] = array(
			'text'  => $this->language->get('text_name_desc'),
			'value' => 'pd.name-DESC',
			'href'  => $this->url->link('product/news', 'sort=pd.name&order=DESC' . $url)
		);

		$data['sorts'][] = array(
			'text'  => $this->language->get('text_price_asc'),
			'value' => 'ps.price-ASC',
			'href'  => $this->url->link('product/news', 'sort=ps.price&order=ASC' . $url)
		);

		$data['sorts'][] = array(
			'text'  => $this->language->get('text_price_desc'),
			'value' => 'ps.price-DESC',
			'href'  => $this->url->link('product/news', 'sort=ps.price&order=DESC' . $url)
		);
		...
		if ($this->config->get('config_review_status')) {
			$data['sorts'][] = array(
				...
				'value' => 'rating-DESC',
				'href'  => $this->url->link('product/news', 'sort=rating&order=DESC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_rating_asc'),
				'value' => 'rating-ASC',
				'href'  => $this->url->link('product/news', 'sort=rating&order=ASC' . $url)
			);
		}
		...
		$data['sorts'][] = array(
				...
				'value' => 'p.model-ASC',
				'href'  => $this->url->link('product/news', 'sort=p.model&order=ASC' . $url)
		);

		$data['sorts'][] = array(
			'text'  => $this->language->get('text_model_desc'),
			'value' => 'p.model-DESC',
			'href'  => $this->url->link('product/news', 'sort=p.model&order=DESC' . $url)
		);
		...
		foreach($limits as $value) {
			$data['limits'][] = array(
				'text'  => $value,
				'value' => $value,
				'href'  => $this->url->link('product/news', $url . '&limit=' . $value)
			);
		}
        ...
        if(!$data['last15']){
		    ...
		    $pagination->url = $this->url->link('product/news', $url . '&page={page}');
		...
		if ($page == 1) {
		    $this->document->addLink($this->url->link('product/news', '', true), 'canonical');
		} else {
		    $this->document->addLink($this->url->link('product/news', 'page='. $page , true), 'canonical');
		}		
		
		if ($page > 1) {
			$this->document->addLink($this->url->link('product/news', (($page - 2) ? '&page='. ($page - 1) : ''), true), 'prev');
		}

		if ($limit && ceil($product_total / $limit) > $page) {
		    $this->document->addLink($this->url->link('product/news', 'page='. ($page + 1), true), 'next');
		}
