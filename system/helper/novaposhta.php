...
class NovaPoshta
{
    ...
    public function update($type)
    {
        ...
        if ($type == "regions") {
            ...
            if ($data) {
                ...
                foreach ($data as $v) {
                    $sql = "INSERT INTO `" . DB_PREFIX . "novaposhta_regions` (`Ref`, `AreasCenter`, `Description`, `DescriptionRu`) VALUES (                        '" . (string) $v["Ref"] . "',\n                        '" . (string) $v["AreasCenter"] . "',\n                        '" . $this->db->escape((string) $v["Description"]) . "', \n\t\t\t\t\t\t'" . $this->db->escape((string) $v["DescriptionRu"]) . "'\n\t\t\t\t\t)";
                    try {
                        $this->db->query($sql);
                        $count++;
                    } catch (Exception $e) {
    ...
    public function getRegionRef($name)
    {
        $id = false;
    ...
    {
        $result = $this->db->query("SELECT `Ref`, `" . $this->description_field . "` as `Description` FROM `" . DB_PREFIX . "novaposhta_regions` ORDER BY `Description`")->rows;
        return $result;
    }
    ...
    {
        $sql = "SELECT *, `" . $this->description_field . "` as `description`,  CONCAT(`Area" . $this->description_field . "`, ' обл.') as `region_description` FROM `" . DB_PREFIX . "novaposhta_cities` WHERE 1";
        if ($region) {
            $sql .= " AND `Area` = '" . $this->db->escape($region) . "'";
        }
        $sql .= " ORDER BY `" . $this->description_field . "`";
    ...
    {
        $results = $this->db->query("SELECT `" . $this->description_field . "` FROM `" . DB_PREFIX . "novaposhta_departments` WHERE `Ref` = '" . $this->db->escape($ref) . "'")->row;
        return $results ? $results[$this->description_field] : "";
    }
    public function getWarehouseByCity($warehouse, $city)
    {
        $warehouse = $this->db->escape($warehouse);
        $city = $this->db->escape($city);
        $result = $this->db->query("SELECT `" . $this->description_field . "` as `Description`, `Ref` FROM `" . DB_PREFIX . "novaposhta_departments` WHERE (`Ref` = '" . $warehouse . "' OR `Description` = '" . $warehouse . "' OR `DescriptionRu` = '" . $warehouse . "') AND (`CityRef` = '" . $city . "' OR `CityDescription` = '" . $city . "' OR `CityDescriptionRu` = '" . $city . "')")->row;
        return $result;
    }
    ...
    {
        $sql = "SELECT *, `" . $this->description_field . "` as `description` FROM `" . DB_PREFIX . "novaposhta_departments` WHERE `CityRef` = '" . $this->db->escape($city) . "'";
        if ($type == "department") {
    ...
    public function getDeparture($products)
    {
        ...
        $data["weight"] = max(round($data["weight"], 2), 0.1);
